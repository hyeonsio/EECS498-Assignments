...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        # at every location in feature map, we shouldn't "lose" any locationsU.
        ######################################################################
        # Fill these.
        stem_cls = []
        stem_box = []
        ...
        for module in [self.stem_cls, self.stem_box]:
            for layer in module:
        ...
        self.pred_cls = nn.Conv2d(stem_channels[-1], self.num_classes, kernel_size = 3, stride = 1, padding = 1)   # Class prediction conv
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for fpn_level, feats in feats_per_fpn_level.items():
          x_cls = self.stem_cls(feats)
          x_box = self.stem_box(feats)
          class_logits[fpn_level] = self.pred_cls(x_cls).permute(0, 2, 3, 1).reshape(x_cls.size(0), -1, self.num_classes)
          boxreg_deltas[fpn_level] = self.pred_box(x_box).permute(0, 2, 3, 1).reshape(x_cls.size(0), -1, 4)
          centerness_logits[fpn_level] = self.pred_ctr(x_box).permute(0, 2, 3, 1).reshape(x_cls.size(0), -1, 1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    deltas = torch.stack([(xc - x1) /  stride, (yc - y1) /  stride, (x2 - xc) /  stride, (y2 - yc) /  stride], dim = 1)

    if gt_boxes.shape[1] == 5:
      bg_mask = (gt_boxes[:, 4] == -1)
      deltas[bg_mask] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    unnormalized_deltas = deltas.clamp_min(0) * stride
    xc, yc = locations[:, 0], locations[:, 1]
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        locations_per_fpn_level = get_fpn_location_coords(
          shape_per_fpn_level = fpn_feats_shapes,
        ...
        for i in range(images.shape[0]):
          matched_boxes_per_image = fcos_match_locations_to_gt(locations_per_fpn_level, self.backbone.fpn_strides, gt_boxes[i])
        ...
        for i in range(images.shape[0]):
          ...
          for fpn_level in ["p3", "p4", "p5"]:            
            gt_deltas_per_image[fpn_level] = fcos_get_deltas_from_locations(locations_per_fpn_level[fpn_level], matched_gt_boxes[i][fpn_level], self.backbone.fpn_strides[fpn_level])
          matched_gt_deltas.append(gt_deltas_per_image)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        # positions to zero.
        ######################################################################
        gt_class = matched_gt_boxes[:, :, 4].to(torch.int64)
        gt_class[gt_class==-1] = self.num_classes # bg
        gt_cls_logits = F.one_hot(gt_class, num_classes = self.num_classes + 1).to(torch.float64)
        gt_cls_logits = gt_cls_logits[:, :, :self.num_classes] # slice out to make bg 0 vector.
        ...
        loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits.view(-1), gt_centerness, reduction="none")
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1: Get the most confidently predicted class and its scor
            level_pred_scores, level_pred_classes = level_pred_scores.max(dim=1)

            # Step 2: Only retain prediction that have a confidence score higher than provided threshold
            ...
            # Step 4:  Clip XYXY box-cordinates. Use `images` to get (height, width) for clipping.
            image_height, image_width = images.size(2), images.size(3)
            level_pred_boxes[:, 0::2] = torch.clamp(level_pred_boxes[:, 0::2], min=0, max=image_width - 1)
            level_pred_boxes[:, 1::2] = torch.clamp(level_pred_boxes[:, 1::2], min=0, max=image_height - 1)
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
