...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.fpn_params['lateral_c3'] = nn.Conv2d(dummy_out_shapes[0][1][1], out_channels, kernel_size = 1, stride = 1, padding = 0)
        self.fpn_params['lateral_c4'] = nn.Conv2d(dummy_out_shapes[1][1][1], out_channels, kernel_size = 1, stride = 1, padding = 0)
        self.fpn_params['lateral_c5'] = nn.Conv2d(dummy_out_shapes[2][1][1], out_channels, kernel_size = 1, stride = 1, padding = 0)
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        yc = yc.to(dtype = dtype, device=device)
        yc = (yc + 0.5) * level_stride
        xc = xc.to(dtype = dtype, device = device)                
        xc = (xc + 0.5) * level_stride
        location_coords[level_name] = torch.stack((xc.view(-1), yc.view(-1)), dim=1)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    while sorted_indices.numel() > 0:
        ...
        keep.append(highest_score_idx.item())
        ...
        x_min = torch.max(current_box[0], other_boxes[:, 0])
        y_min = torch.max(current_box[1], other_boxes[:, 1])
        x_max = torch.min(current_box[2], other_boxes[:, 2])
        y_max = torch.min(current_box[3], other_boxes[:, 3])
