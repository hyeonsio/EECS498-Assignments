          ...
          for f in range(F):
            for i in range(H_prime):
              for j in range(W_prime):
                out[n, f, i, j] = torch.sum(x_pad[n,:,i*stride:i*stride+HH,j*stride:j*stride +WW]*w[f]) + b[f]        
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def forward(x, pool_param):
        ...
        for n in range(N):
          for c in range(C):
            for i in range(H_prime):
              for j in range(W_prime):
                out[n, c, i, j] = x[n,c,i*stride:i*stride+pool_height,j*stride:j*stride +pool_width].max()
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
          for c in range(C):
            for i in range(H_prime):
              for j in range(W_prime):
                receptive_feild = x[n,c,i*stride:i*stride+pool_height,j*stride:j*stride +pool_width]
                mask = (receptive_feild == receptive_feild.max())
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        pool_width = 2
        pool_stride = 2
        H_conv = (H - filter_size + 2 * conv_pad) // conv_stride + 1
        W_conv = (W - filter_size + 2 * conv_pad) // conv_stride + 1
        H_pool = (H - pool_height) // pool_stride + 1
        W_pool = (W - pool_width) // pool_stride + 1

        self.params['W2'] = weight_scale * torch.randn(num_filters*H_pool*W_pool, hidden_dim, device = device, dtype = dtype)
    ...
    def loss(self, X, y=None):
        ...
        da1, grads['W3'], grads['b3'] = Linear.backward(dscores, cache3)
        da2, grads['W2'], grads['b2'] = Linear_ReLU.backward(da1, cache2)
        dx, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(da2, cache1)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(1, self.num_layers):
            in_dim = C if i == 1 else num_filters[i-2]
            out_dim = num_filters[i-1]
            if weight_scale == 'kaiming':
                self.params[f'W{i}'] = kaiming_initializer(in_dim, out_dim, K=3, device=device, dtype=dtype)
            else:
                self.params[f'W{i}'] = weight_scale * torch.randn(out_dim, in_dim, 3, 3, device=device, dtype=dtype)
            
            if self.batchnorm:
                self.params[f'gamma{i}'] = torch.ones(out_dim, device=device, dtype=dtype)
                self.params[f'beta{i}'] = torch.zeros(out_dim, device=device, dtype=dtype)
            
            self.params[f'b{i}'] = torch.zeros(out_dim, device=device, dtype=dtype)
            
        final_in_dim = num_filters[-1] * (H // 2 ** len(self.max_pools)) * (W // 2 ** len(self.max_pools))
        if weight_scale == 'kaiming':
            self.params[f'W{self.num_layers}'] = kaiming_initializer(final_in_dim, num_classes, device=device, dtype=dtype)
        else:
            self.params[f'W{self.num_layers}'] = weight_scale * torch.randn(final_in_dim, num_classes, device=device, dtype=dtype)
    ...
    def loss(self, X, y=None):
        ...
        loss += self.reg * torch.sum(self.params[f'W{self.num_layers}']*self.params[f'W{self.num_layers}'])   
        if self.batchnorm:
          for i in range(self.num_layers - 1, 0, -1):
            if i-1 in self.max_pools:
                dout, grads[f"W{i}"], grads[f"b{i}"], grads[f'gamma{i}'], grads[f'beta{i}'] = Conv_BatchNorm_ReLU_Pool.backward(dout, caches.pop())
            else:
                dout, grads[f"W{i}"], grads[f"b{i}"], grads[f'gamma{i}'], grads[f'beta{i}'] = Conv_BatchNorm_ReLU.backward(dout, caches.pop())
          ...
          for i in range(self.num_layers - 1, 0, -1):
              if i-1 in self.max_pools:
                  dout, grads[f"W{i}"], grads[f"b{i}"] = Conv_ReLU_Pool.backward(dout, caches.pop())
              else:
                  dout, grads[f"W{i}"], grads[f"b{i}"] = Conv_ReLU.backward(dout, caches.pop())
              loss += self.reg * torch.sum(self.params[f'W{i}']*self.params[f'W{i}'])    
              grads[f"W{i}"] += 2 * self.reg * self.params[f"W{i}"]    
        #############################################################
        #                       END OF YOUR CODE                    #
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(input_dims=input_dims, num_classes=10,
                        num_filters=([8] * 2) + ([16] * 2) + ([32] * 2) + ([64] * 1), 
                        max_pools=[1, 3, 5],
    ...
    solver = Solver(model, data_dict,
                    ...
                    lr_decay = 0.9, # 0.9 715
                    print_every=50, device=device)  # 20  
    #########################################################
    #                  END OF YOUR CODE                     #
    #########################################################
    return solver
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            cache = (x, x_normalized, sample_mean, sample_var, gamma, beta, eps, mode)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
            ...
            cache = (x, x_normalized, running_mean, running_var, gamma, beta, eps, mode)
            ################################################################
            #                      END OF YOUR CODE                        #
            ################################################################
        else:
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        x, x_normalized, mean, var, gamma, beta, eps, mode = cache
        N, D = x.shape

        dgamma = torch.sum(dout * x_normalized, dim=0)
        dbeta = torch.sum(dout, dim=0)
        dx_normalized = dout * gamma
        dx = dx_normalized / torch.sqrt(var + eps)

        if mode == 'train':
          # x_normalized = (x - sample_mean) / torch.sqrt(sample_var + eps)
          dvar = torch.sum(dx_normalized * (x - mean) * (-0.5) * torch.pow(var + eps, -1.5)  , dim = 0)        
          dmean = torch.sum((-1) * dx_normalized / torch.sqrt(var + eps) + (-2) * (x- mean) * dvar, dim = 0) 
          dx = dx + 2 * (x- mean) * dvar / N + dmean / N
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        ###################################################################
        x, x_normalized, sample_mean, sample_var, gamma, beta, eps, mode = cache
        N, D = x.shape
        dx_normalized = dout * gamma
        dx = (1. / N) * (1. / torch.sqrt(sample_var + eps)) * (N * dx_normalized - torch.sum(dx_normalized, dim=0) - x_normalized * torch.sum(dx_normalized * x_normalized, dim=0))
        dgamma = torch.sum(dout * x_normalized, dim=0)
        dbeta = torch.sum(dout, dim=0)
        #################################################################
        #                        END OF YOUR CODE                       #
