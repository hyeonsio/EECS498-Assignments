...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        ######################################################################
        out = x.reshape(x.size(0), -1)  # Reshape input into rows
    ...
    def backward(dout, cache):
        ...
        ##################################################
        x_reshape = x.reshape(x.size(0), -1)
        
        dx = dout.matmul(w.T).reshape(x.shape)
        dw = x_reshape.T.matmul(dout)
        db = dout.sum(dim=0)        
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # scores variable.                                          #
        #############################################################        
        h1, cache1 = Linear.forward(X, self.params['W1'], self.params['b1'])
        a1, cache_relu = ReLU.forward(h1)
        ...
        da1, grads['W2'], grads['b2'] = Linear.backward(dscores, cache2)
        dh1 = ReLU.backward(da1, cache_relu)
        dx, grads['W1'], grads['b1'] = Linear.backward(dh1, cache1)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            dout, grads[f"W{i}"], grads[f"b{i}"] = Linear.backward(dout, caches.pop())
...
def create_solver_instance(data_dict, dtype, device):
    ...
    #############################################################
    solver = None        
    solver = Solver(model, data_dict, optim_config  = {"learning_rate": 2e-1}, lr_decay = 0.95, num_epochs=20, device=device)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
