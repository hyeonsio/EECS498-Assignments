...
def svm_loss_vectorized(
    ...
    scores = X.mm(W) # (N, C)
    correct_class_score = scores[torch.arange(num_train), y].unsqueeze(1) # (N, 1)
    margin = torch.clamp(scores - correct_class_score + 1, min = 0) # (N, C)
    margin[torch.arange(num_train), y] = 0
    
    loss = torch.sum(margin) / num_train # (N,)
    loss += reg * torch.sum(W * W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    # loss.                                                                     #
    #############################################################################
    mask = (margin > 0).to(torch.float64) # (N, C)
    
    count = torch.sum(margin, dim=1) # (N,)
    mask[torch.arange(num_train), y] = - count 
  
    dW = X.t().mm(mask) / num_train + 2 * reg * W # (N, D) * (D, C)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def svm_get_search_params():
    ...
    regularization_strengths = [1e-4, 5e-3, 1e-3, 1e-2, 1e-1]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    ###########################################################################    
    cls.W, _ = train_linear_classifier(svm_loss_vectorized, 
                            None, data_dict['X_train'], 
                            data_dict['y_train'],
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
      scores_i = scores[i]
      scores_i -= torch.max(scores_i)
      exp_scores_i = torch.exp(scores_i)
      softmax = exp_scores_i / torch.sum(exp_scores_i)
      loss -= torch.log(softmax[y[i]]) 

      # gradient = softmax(x) - 1 if x==true class else softmax
      softmax[y[i]] -= 1
      dW += X[i].unsqueeze(1).mm(softmax.unsqueeze(0))
...
def softmax_loss_vectorized(
    ...
    softmax = exp_scores / torch.sum(exp_scores, dim=1, keepdim=True)
    
    loss = -torch.log(softmax[torch.arange(num_train),y]).mean() + reg * torch.sum(W * W)
    
    # gradient = softmax(x) - 1 if x==true class else softmax
    softmax[torch.arange(num_train), y] -= 1
    dW = X.t().mm(softmax.mean(dim=1, keepdim=True)) + 2 * reg * W    
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
...
def softmax_get_search_params():
    ...
    # classifier.                                                             #
    ###########################################################################
    learning_rates = [2.5e-3, 2e-3, 4e-3, 35e-4, 3e-3]
    regularization_strengths = [1e-5, 1e-4, 1e-3, 5e-5, 3e-3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
