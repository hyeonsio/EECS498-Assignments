...
def nn_forward_backward(
    ...
    loss = -torch.log(softmax[torch.arange(num_train), y]).mean() + reg * (torch.sum(W1 * W1) + torch.sum(W2 * W2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    # tensor of same size                                                     #
    ###########################################################################
    softmax[torch.arange(num_train), y] -= 1    # (N, C)
    softmax /= num_train
        
    grads["b2"] = softmax.sum(dim=0)
    grads["W2"] = h1.t().mm(softmax) + 2 * reg * W2  # (H, N) * (N, C)
    
    grads_h1 = softmax.mm(W2.t()) # (N, C) * (C, H)
    grads_h1[h1 <= 0] = 0

    grads["b1"] = grads_h1.sum(dim=0)
    grads["W1"] = X.t().mm(grads_h1) + 2 * reg * W1 # (D, N) * (N, H)
    # ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_predict(
    ...
    ###########################################################################
    y_pred = nn_forward_pass(params, X)[0].argmax(dim=1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
...
def nn_get_search_params():
    ...
    # classifier.                                                             #
    ###########################################################################
    learning_rates = [1.18, 1.15, 1.12] 
    hidden_sizes = [2048, 1024] 
    regularization_strengths = [1e-3, 1e-4] # 3
    learning_rate_decays = [0.99, 0.98, 0.97] # 3
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    ...
    for lr in learning_rates:
      for hs in hidden_sizes:
        for reg in regularization_strengths:
          for dr in learning_rate_decays:
              print('train with learning rate: {}, hidden size: {}, regularization strength: {}, learning_rate_decay: {} '.format(lr, hs, reg, dr))
              ...
              stats = net.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'],
                        ...
                        learning_rate=lr, learning_rate_decay=dr,
