...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Euclidean Distance:  xtr**2 -2xtr*xtst + xtst**2     
    train_norms = torch.sum(x_train ** 2, dim = 1, keepdim=True) # (num_train, 1) 
    test_norms = torch.sum(x_test ** 2, dim = 1) # (num_test, 1)
    dot_product = torch.matmul(x_train, x_test.t()) # (num_train, num_test)
    dists = torch.sqrt(train_norms - 2 * dot_product + test_norms)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
      ...
      _, indices = torch.topk(dists_j, k, largest=False)
      labels = y_train[indices]      
      unique_labels, label_counts = torch.unique(labels, return_counts=True)

      max_count = torch.max(label_counts)
      label_candidates = unique_labels[label_counts == max_count]
      predicted_label = torch.min(label_candidates)
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for fold in range(num_folds):
        ...
        x_train_current_fold = torch.cat(x_train_folds[:fold] + x_train_folds[fold + 1:], dim = 0)
        y_train_current_fold = torch.cat(y_train_folds[:fold] + y_train_folds[fold + 1:], dim = 0)

        knn = KnnClassifier(x_train_current_fold, y_train_current_fold)
        
        y_val_pred = knn.predict(x_val, k)

        accuracy = torch.mean((y_val_pred == y_val).float()).item()
