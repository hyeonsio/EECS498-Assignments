...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    y = x.clone()
    y[range(x.size(0)), y.argmin(dim=1)] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    y = x.clone()
    row, col = y.size()
    column_means = torch.sum(y, dim=0) / row
    column_std = torch.sqrt(torch.sum((y - column_means) ** 2, dim=0) / (row - 1))
    y = (y - column_means) / column_std
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    cumulative_sum = torch.cumsum(torch.cat(xs), dim=0)
    indices = torch.cat([torch.zeros(1, dtype=torch.long), ls.cumsum(dim=0)[:-1]])
    y = (cumulative_sum[indices[1:]] - cumulative_sum[indices[:-1]]) / ls.float()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
